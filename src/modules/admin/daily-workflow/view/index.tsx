...
const ViewDailyWorkflowModule: FC<ViewDailyWorkflowModuleProps> = ({ report }) => {
    const [reports, setReports] = useState<IReport[]>(report.reports)
    const { token } = useUserContext();
    const { update } = useDailyWorkflow({ token })
                    ...
                    <div className="flex justify-between flex-col lg:flex-row">
                        ...
                        <div className="flex gap-3">
                            <ConfirmDialog
                                ...
                                <Button type="button" variant={'destructive'}>Discard Report</Button>
                            </ConfirmDialog>
                            <Button type="button" onClick={handleSave}>Update Report</Button>
                        </div>
                    </div>
                </Card>
                <ReportTable reports={reports} onDelete={handleDeleteReport} onEdit={handleEditReport} />
            </form>
        </Form>
    );
};
